...
assert (
    num_heads * head_size
) == n_embd, "Embedding size must be divisible by number of heads"
...
class SimpleTranslator:
    ...
    def translate_parallel(self, sentence):
        ...
        Q = torch.stack(
            [self.one_hot_in[token] for token in tokens if token in self.one_hot_in]
        )
        ...
        # Decode all outputs
        translated_tokens = [
            self.decode_one_hot(self.one_hot_out, output) for output in outputs
        ]
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        head_outputs = [
            head(x)[0] for head in self.heads
        ]  # Get outputs, ignore attention weights
...
class SimpleTransformer(nn.Module):
    ...
    def __init__(
        self, vocab_size, n_embd, num_heads, n_layers, max_length=1000, dropout=0.0
    ):
        super().__init__()
